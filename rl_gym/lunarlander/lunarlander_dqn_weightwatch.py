...
for ep in range(num_episodes):
	...
	for t in range(max_steps_ep):
		...
		# to observe the effect of how the weights change as reward changes, reset epsilon on reward change so that there's enough exploration
		if ep == target_episodes:
			epsilon = epsilon_start
			steps_eps = 0
		# linearly decay epsilon from epsilon_start to epsilon_end over epsilon_decay_length steps	
		elif steps_eps < epsilon_decay_length:
			epsilon -= epsilon_linear_step
		# then exponentially decay it every episode
		elif done:
			epsilon *= epsilon_decay_exp

		if steps_eps == epsilon_decay_length:
